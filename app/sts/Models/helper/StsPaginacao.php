...
{   
    ...
    private function layoutPaginacao()
    {
        ...
        $this->Resultado .= "<li class='page-item m_paginacao'>";
        ...
        for($iPag = $this->Pagina - $this->MaxLinks; $iPag <= $this->Pagina - 1; $iPag++){
            if($iPag >= 1) {
                $this->Resultado .= "<li class='page-item m_paginacao'>";
                $this->Resultado .= "<a class='page-link' href=\"{$this->Link}?pg={$iPag}\">$iPag</a>";
                $this->Resultado .= "</li>";
            }
        }

        $this->Resultado .= "<li class='page-item active'>";
        $this->Resultado .= "<a class='page-link' href=\"{$this->Link}?pg={$iPag}\">{$this->Pagina}</a>";
        $this->Resultado .= "</li>";

        for($dPag = $this->Pagina + 1; $dPag <= $this->Pagina + $this->MaxLinks; $dPag++){
            if($dPag <= $this->TotalPaginas) {
                $this->Resultado .= "<li class='page-item m_paginacao'>";
                $this->Resultado .= "<a class='page-link' href=\"{$this->Link}?pg={$dPag}\">$dPag</a>";
                $this->Resultado .= "</li>";
            }
        }
        $this->Resultado .= "<li class='page-item m_paginacao'>";
